...
def main():
    ...
    base_path = args.path
    img_path = args.video
    images = [f for f in sorted(os.listdir(img_path)) if f.endswith('.jpg')]
    img_path = os.path.join(img_path, 'data/test_data/images')
    ...
    for img in images:
        ...
        output_path = os.path.join(output_dir, img)  # Keep the original filename
